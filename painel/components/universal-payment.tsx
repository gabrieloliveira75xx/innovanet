...
export function UniversalPayment({ 
  items, 
  isOpen, 
  onClose, 
  onPaymentSuccess,
  title = "Pagamento",
  description = "Complete seu pagamento de forma segura"
}: UniversalPaymentProps) {
  ...
  const totalAmount = items.reduce((total, item) => total + (item.amount * (item.quantity || 1)), 0)
      ...
      <div className="w-full max-w-4xl mx-auto my-4">
        <Card className="shadow-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="bg-gray-50 rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-3">
            <h4 className="font-semibold text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">Resumo do Pedido</h4>
            {items.map((item, index) => (
              <div key={index} className="flex justify-between items-start gap-2">
                <div className="flex-1 min-w-0">
                  <span className="font-medium text-gray-900 text-sm sm:text-base block truncate">{item.title}</span>
                  {item.description && (
                ...
                <span className="font-semibold text-gray-900 text-sm sm:text-base flex-shrink-0">
                  R$ {(item.amount * (item.quantity || 1)).toFixed(2).replace('.', ',')}
                </span>
              </div>
            ))}
